// ReconLens — background.js (Cross-browser & MV3 Persistence)
...
function extract_info(data) {
  ...
  var extract_data = {}
  extract_data['id_card'] = data.match(/['"]((\d{8}(0\d|10|11|12)([0-2]\d|30|31)\d{3}$)|(\d{6}(18|19|20)\d{2}(0[1-9]|10|11|12)([0-2]\d|30|31)\d{3}(\d|X|x)))['"]/g);
  extract_data['mobile'] = data.match(/['"](1(3([0-35-9]\d|4[1-8])|4[14-9]\d|5([\d]\d|7[1-79])|66\d|7[2-35-8]\d|8\d{2}|9[89]\d)\d{7})['"]/g);
  extract_data['email'] = data.match(/['"][a-zA-Z0-9\._\-]*@[a-zA-Z0-9\._\-]{1,63}\.((?!js|css|jpg|jpeg|png|ico)[a-zA-Z]{2,})['"]/g);
  ...
  extract_data['sensitive'] = get_secret(data);
  if (extract_data['url']) {
...
function webhook(data) {
  ...
  chrome.storage.local.get(["webhook_setting"], function (settings) {
    if (!settings || !settings["webhook_setting"] || Object.keys(settings["webhook_setting"]).length === 0) {
      return;
    }
...
chrome.runtime.onMessage.addListener(
  function (request, sender, sendResponse) {
    ...
    if (request.greeting == "find") {
      ...
      // Memory-first strategy: return from search_data cache if available
      if (search_data[request.current]) {
        sendResponse(search_data[request.current]);
      } else {
        chrome.storage.local.get(["findsomething_result_" + request.current], function (result) {
          const data = result["findsomething_result_" + request.current];
          if (data) {
            search_data[request.current] = data; // Hydrate cache
          }
          sendResponse(data);
        });
      }
